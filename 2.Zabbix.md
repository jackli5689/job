...
然后使用windows下装的java JDK安装目录中/bin/jconsole进行连接测试看有没有问题，选择远程进程，输入开启jmx的IP加端口，例：192.168.1.233:8888,能连接看到信息则表示开启成功,最后在zabbix-web上加入tomcat的jmx类型监控
...
[root@zabbix2 zabbix-3.0.28]# yum install -y mariadb-server mariadb mariadb-devel
[root@zabbix2 download]#  cd zabbix-3.0.28/
[root@zabbix2 zabbix-3.0.28]# ./configure --prefix=/usr/local/zabbix --enable-server --enable-agent --with-mysql --enable-ipv6 --with-net-snmp --with-libcurl --with-libxml2  #编辑安装zabbix server和agent
...
[root@zabbix2 zabbix-3.0.28]# yum install -y net-snmp-devel 
configure: error: Curl library not found
[root@zabbix2 zabbix-3.0.28]# yum install libcurl-devel -y
[root@zabbix2 zabbix-3.0.28]# make install 
[root@zabbix2 zabbix-3.0.28]# sbin/zabbix_server
sbin/zabbix_server: error while loading shared libraries: libmysqlclient.so.18: cannot open shared o
[root@zabbix2 sbin]# locate libmysqlclient.so.18
/usr/lib64/mysql/libmysqlclient.so.18
/usr/lib64/mysql/libmysqlclient.so.18.0.0
[root@zabbix2 sbin]# cat /etc/ld.so.conf.d/mysql.conf 
/usr/local/mysql/lib
[root@zabbix2 sbin]# ldconfig
