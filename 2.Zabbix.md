...
参考链接：https://wiki.jiankongbao.com/doku.php
linux snmp oid #百度搜索
#监控对象识别：
1. 使用SNMP监控交换机
2. 使用IPMI监控服务器硬件
...
snmp-get、snmp-set
snmptranslate:可以将MID和OID两种表现形式进行转换
...
snmpwalk -v 2c -c route 192.168.1.1 SNMPv2-MIB::sysDescr.0
...
#创建触发器：
选中主机，并选中触发器，新建触发器，选择一个处理函数（例如：last(),max()等），就会触发触发器做动作，动作就是我们定义的信息、条件、和操作（发送给谁，哪种方式）
名称：Nginx Active > 1
...
###############
添加媒介为脚本类型，指定名称脚本 sms.sh（自己会去AlertScriptsPath=/usr/lib/zabbix/alertscripts查找），如有需要添加脚本参数：{ALERT.SENDTO}[用户属性中的媒体]、{ALERT.SUBJECT}[动作里面的主题]、{ALERT.MESSAGE}[动作的信息]
...
zabbix-proxy他zabbix-agent也可以设置主动和被动模式
