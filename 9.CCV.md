...
安装虚拟机（支持热部署）：
[root@Linux-node1-salt opt]# virt-install --virt-type kvm --name CentOS7-x86_64 --memory=2048,maxmemory=2048 --vcpus=1,maxvcpus=2 --cdrom=/opt/Centos7.5.iso --disk path=/opt/CentOS7-x86_64.qcow2 --network bridge=br0 --graphics vnc,listen=0.0.0 --noautoconsole
