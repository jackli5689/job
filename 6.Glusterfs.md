...
#构建企业级分布式存储
1.硬件选型
一般2U的机型，SATA磁盘4G，I/O要求高可选SSD，为了充分保证系统稳定性和性能，要求所有gluster服务器配置尽量一致，尤其是硬盘数量和大小。机器的RAID卡需要带电池，缓存越大，性能越好，一般情况下做RAID10，出于空间考虑RAID5也可以，但是热备盘要1-2块。
2.系统要求及分区划分
...
1. 先移除storage2坏的那块硬盘，然后添加同规格的硬盘，进行xfs文件格式化、挂载。
2. 在storage1服务器使用命令：getfattr -d -m '.*' /storage/brick2 获取隐藏的gluster配置文件详细信息（trusted.glusterfs.volume-id -v 0slEaIKclQQ0GpwUXSYvtHqw== /storage/brick2）
3. 在storage2服务器/storage/brick2目录上（复制卷的挂载目录）执行：setfattr -n trusted.glusterfs.volume-id -v 0slEaIKclQQ0GpwUXSYvtHqw== /storage/brick2 使新硬盘恢复隐藏的gluster配置文件，
4. 在使用命令：getfattr -d -m '.*' /storage/brick2 可查看恢复的信息
5. 重试上面的步骤，把gluster想关的隐藏文件一一恢复，不管gluster文件的顺序，quota限额这项可不用复制，重启gluster服务就会自动同步
...
解决办法：
准备一台服务器跟坏的服务器配置一样，至少硬盘大小和数量一样，然后安装系统和gluster软件。再在其他健康的节点上使用命令gluster peer status查看所有的节点UUID，包括坏的那台服务器节点UUID，可以在新安装的这台服务器上修改文件:/var/lib/glusterd/glusterd.info的UUID成之前坏的那台UUID，然后格式化和挂载硬盘，然后执行:gluster volume heal gv2 full启动自动同步修复。
！！注意：如果硬盘没坏，只是系统还的话，就在修改为UUID，格式化和挂载硬盘后直接添加之前的硬盘，因为硬盘下有gluster的隐藏配置文件，直接添加即可。
