...
###服务启动用户规范：
	所有服务，统一使用www用户，uid为666，除负载均衡需要监听80端口使用的root启动外，所有服务必须使用www用户启动，使用大约1024的端口。
...
8.	重启cobbler的服务
9.	openssl passwd -1 -salt 'cobler' 'cobler' 生成加密密码，并在/etc/cobbler/settings将default_password_crypted 值设为刚才生成的加密密码，此密码为自动化部署成功后root密码
10.	vim /etc/cobbler/dhcp.template ，设置如下：
...
导入后的镜像路径在/var/www/cobbler/ks_mirror/下
